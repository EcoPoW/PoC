...
def new_block(seq):
    ...
    longest = longest_chain()
    if longest:
        leaders = set([("localhost", i.identity) for i in longest[-4:-3]])
        leader.update(leaders)
    if height - 3 > 0:
        leader.current_view = height - 3
...
def new_tx_block(seq):
    ...
    try:
        ...
        sql = "INSERT INTO data"+tree.current_port+" (txid, timestamp, hash, data) VALUES (%s, %s, %s, %s)"
        database.connection.execute(sql, txid, int(timestamp), block_hash, tornado.escape.json_encode(transaction))
    except:
        pass
